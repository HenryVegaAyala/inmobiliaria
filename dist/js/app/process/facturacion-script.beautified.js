$(function() {
    ...
    $("#groupVistaConsumo").on("click", ".span-button", function(event) {
        ...
        $(this).addClass("btn-success");
        if (targedMainId == "#tabCalculoEscalonable") ListarPropiedadConsumo(targedId); else if (targedMainId == "#tabConceptoVariable") {
            if ($("#hdIdConcepto").val() > "0") ListarPropiedadConsumo_Concepto(targedId);
        } else if (targedMainId == "#tabCalculoAscensor") ListarTorreSuministro();
    });
    ...
    $("#pnlInfoProyecto").add("#pnlConsumoProyecto").add("#pnlInfoConcepto").on("click", function(event) {
        ...
        if (tipofiltro == "proyecto" || tipofiltro == "consumo" || tipofiltro == "concepto") {
            ...
            if (tipofiltro == "concepto") ListarConcepto("1"); else ListarProyectos("1");
        }
    ...
    $("#btnImprimirFactura").on("click", function(event) {
        ...
        MessageBox("¿Que desea hacer?", "M&oacute;dulo de impresi&oacute;n", "[Salir sin cambios], [Ver impresion], [Generar impresion]", function(action) {
            if (action == "Ver impresion") {
                VerImpresion("media/pdf/" + $("#hdIdProyecto").val() + $("#hdAnho").val() + $("#hdMes").val() + ".pdf");
    ...
    $("#tablePropiedad tbody").on({
        ...
        keyup: function(event) {
            if ($(this).hasClass("lecanterior") || $(this).hasClass("lecactual")) {
                ...
                $(this).parent().parent().find(".consumo").text(consumo.toFixed(2));
                $(this).parent().parent().find(".input-consumo").val(consumo.toFixed(2));
            }
        }
    }, "input:text");
    ...
    $("#btnCloseGenPDF").add("#btnCancelarEnvio").on("click", function(event) {
        ...
        if (completado == false) {
            ...
            if (confirma) {
                progress = 0;
                indexList = 0;
                if (intervalIndividual.isRunning()) intervalIndividual.stop();
...
function IniciarForm() {
    ...
    var escobrodiferenciado = getParameterByName("escobrodiferenciado");
    ...
    if (escobrodiferenciado == "0") {
        ...
        if ($("#tabCalculoAscensor").is(":visible")) $("#groupOpcionesFactura .btn:first").trigger("click");
    } else $("#btnAscensor").removeClass("oculto");
    if ($("#tabFacturacion").is(":visible")) {
        paginaGenFacturacion = 1;
        ListarFacturacion("#gvGenFacturacion", "1");
    } else $("#groupVistaConsumo .btn.btn-success").trigger("click");
}
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                ...
                if (selectorgrid == "#gvGenFacturacion") {
                    ...
                    $("#btnExportarFacturasExcel, #btnImportarDetalleFacturas, #btnRegenerar, #btnEliminar, #btnVistaPrevia, #btnGuardarConcepto, #btnDivisionFactura, #btnEnviarEmail, #btnImprimirFactura").removeClass("oculto");
...
function ListarPropiedadConsumo(tipo) {
    ...
    tipobusqueda = tipo == "consulta" ? "2" : "1";
    $.ajax({
        url: "services/facturacion/facturacion-search.php",
        type: "GET",
        dataType: "json",
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    var consumo = Number(data[i].td_consumoperiodo).toFixed(2);
                    strhtml += "<tr>";
                    strhtml += "<td>";
                ...
                $("#btnCalcularConsumo").removeClass("oculto");
            } else $("#btnCalcularConsumo").addClass("oculto");
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    strhtml += "<tr>";
                    strhtml += "<td>";
                    strhtml += '<input type="hidden" name="consumo[' + i + '][iditem]" value="' + data[i].td_idconsumoconcepto + '" />';
                    strhtml += '<input type="hidden" name="consumo[' + i + '][idpropiedad]" value="' + data[i].tm_idpropiedad + '" />';
                    strhtml += data[i].tm_descripcionpropiedad + "</td>";
            ...
            } else $("#btnCalcularConsumo_Concepto").addClass("oculto");
...
function RegistrarCalculoConsumo() {
    ...
    // var consumo = '0';
    precargaExp("#pnlDetalle > .gp-body", true);
    // itemsDetalle = $('#tablePropiedad .ibody table');
    // tablePropiedad = itemsDetalle[0];
    // countdata = tablePropiedad.rows.length;
    // if (countdata > 0){
    //     while (i < countdata){
    //         iditem = tablePropiedad.rows[i].getAttribute('data-iddetalle');
    //         idpropiedad = tablePropiedad.rows[i].getAttribute('data-idpropiedad');
    //         lecanterior = tablePropiedad.rows[i].cells[1].childNodes[0].value;
    //         lecactual = tablePropiedad.rows[i].cells[2].childNodes[0].value;
    //         consumo = tablePropiedad.rows[i].cells[3].innerText;
    //         fechaini = tablePropiedad.rows[i].cells[4].childNodes[0].value;
    //         fechafin = tablePropiedad.rows[i].cells[5].childNodes[0].value;
    //         if (Number(consumo) > 0){
...
function RegistrarCalculoConsumo_Concepto() {
    ...
    // var importe = '0';
    precargaExp("#pnlDetalle > .gp-body", true);
    // itemsDetalle = $('#tablePropiedadConcepto .ibody table');
    // tablePropiedad = itemsDetalle[0];
    // countdata = tablePropiedad.rows.length;
    // if (countdata > 0){
    //     while (i < countdata){
    //         iditem = tablePropiedad.rows[i].getAttribute('data-iddetalle');
    //         idpropiedad = tablePropiedad.rows[i].getAttribute('data-idpropiedad');
    //         importe = tablePropiedad.rows[i].cells[1].innerText;
    //         if (Number(importe) > 0){
                ...
                if (data.length > 0) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            $("#ifrImpresionFactura").attr("src", "media/pdf/" + folder + item.tm_per_ano + item.tm_per_mes + "/" + item.tm_idfacturacion + ".pdf");
                        },
                        error: function(data) {
                            console.log(data);
                        }
...
function executeImport() {
    ...
    intervalProgress = new Interval(function() {
        ...
        if (progress == 100) {
            ...
            if (progressSuccess) intervalProgress.start();
        }
    }, 100);
    pbMetro.progressbar("value", "0");
    pbMetro.progressbar("color", "bg-cyan");
    ...
    $.ajax({
        ...
        success: function(data) {
            progressError = false;
            if (data.rpta != "0") progressSuccess = true;
            ...
            MessageBox(data.titulomsje, data.contenidomsje, "[Aceptar]", function() {
                if (data.rpta != "0") {
                    ...
                    cancelImport();
                    if (tipoImport == "00") ListarPropiedadConsumo("consulta"); else if (tipoImport == "01") ListarPropiedadConsumo_Concepto("consulta"); else ListarFacturacion("#gvGenFacturacion", "1");
                }
            });
        },
        beforeSend: function() {

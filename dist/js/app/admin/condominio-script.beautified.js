$(function() {
    ...
    $("#myTab").on("click", "a", function(e) {
        ...
        _tab = _tab.substr(1);
        if (_tab != "propiedades") createFrame("#pnlInfoGeneralProyecto", this);
    });
    ...
    $("#tableProceso tbody").on("click", "tr button", function(event) {
        ...
        var _row = getParentsUntil(this, "#tableProceso", "tr");
    ...
    $("#gvDatos").on("click", ".dropdown-list a", function(event) {
        ...
        $("#hdIdProyecto").val(idproyecto);
        if (accion == "edit") GoToEdit(idproyecto); else if (accion == "more-info") {
            $("#pnlInfoGeneralProyecto iframe").remove();
            $("#pnlInfoDetalleProyecto iframe").remove();
            $("#pnlListado").fadeOut(400, function() {
                $("#pnlOpciones").fadeIn(400, function() {
                    GoToPropiedades(proyecto[0]);
                    $("#myTabs li.active").removeClass("active");
                    $("#pnlInfoGeneralProyecto .tab-pane.active").removeClass("active");
                    $("#myTabs li:first-child").addClass("active");
                    $("#pnlInfoGeneralProyecto .tab-pane:first").addClass("active");
            ...
            bootbox.confirm("¿Desea eliminar este proyecto?", function(result) {
                if (result) EliminarItemProyecto(proyecto[0]);
            });
        }
    });
    // $('#gvDatos').on('click', '.dato', function(event) {
    ...
    $("#gvPropiedad").on("click", ".tile", function(event) {
        ...
        if ($(this).hasClass("selected")) {
            ...
            if (tipovaloracion == "02") inputRatio.removeClass("hide"); else if (tipovaloracion == "03") inputImporteFijo.removeClass("hide");
    ...
    $("#btnSelectAllPropiedades").on("click", function(event) {
        ...
        if (tipovaloracion == "02") $("#gvPropiedad .inputRatio").removeClass("hide"); else if (tipovaloracion == "03") $("#gvPropiedad .inputImporteFijo").removeClass("hide");
...
function ListarAnhoProceso() {
    $.ajax({
        ...
        success: function(data) {
            ...
            } else strhtml = '<option value="0">NO HAY PROCESOS VIGENTES</option>';
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    ...
                    strhtml += 'data-idproyecto="' + iditem + '" data-codigo="' + data[i].codigoproyecto + '" data-tipoproyecto="' + data[i].tipoproyecto + '" data-datosimpleduplex="' + data[i].datosimpleduplex + '" data-tipovaloracion="' + data[i].tipovaloracion + '" data-escobrodiferenciado="' + data[i].escobrodiferenciado + '" ';
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    ...
                    if (idtipopropiedad == "DPT") colortile = "bg-white"; else if (idtipopropiedad == "DEP") colortile = "bg-blueGrey"; else if (idtipopropiedad == "EST") colortile = "bg-indigo";
                    ...
                    strhtml += '<input type="text" class="input-saldoinicial" value="' + data[i].saldoinicial + '" />';
                    ...
                    strhtml += '<div class="input-control text no-margin" data-role="input-control">';
                    strhtml += '<input type="text" class="input-ratio" value="' + data[i].ratio + '" />';
                    strhtml += '<button class="btn-clear" tabindex="-1" type="button"></button>';
                    strhtml += "</div>";
                    strhtml += "</div>";
                    ...
                    strhtml += '<div class="input-control text no-margin" data-role="input-control">';
                    strhtml += '<input type="text" class="input-importefijo" value="' + data[i].importefijo + '" />';
                    strhtml += '<button class="btn-clear" tabindex="-1" type="button"></button>';
                    strhtml += "</div>";
                    strhtml += "</div>";
                    strhtml += "</div>";
...
function EnvioDataAsignacion(item) {
    ...
    var pbMetro;
    var itemSelected;
    var itemsPropiedades;
    itemSelected = $(item);
    progressBar = itemSelected.find(".progress-bar");
    pbMetro = progressBar.progressbar({
        animate: true,
        color: "bg-cyan"
    });
    tipopersona = itemSelected.attr("data-tipopersona");
    if (tipopersona == "00") idpersona = itemSelected.attr("data-idpropietario"); else if (tipopersona == "01") idpersona = itemSelected.attr("data-idinquilino");
    itemsPropiedades = $("#gvPropiedad .tile.selected");
    listpropiedades = $.map(itemsPropiedades, function(n, i) {
...
function EnvioDataEditPropiedad(item) {
    ...
    idpropiedad = item.getAttribute("data-idpropiedad");
    idtipopropiedad = item.getAttribute("data-idtipopropiedad");
    itemSelected = $(item);
    descripcionpropiedad = itemSelected.find(".input-descripcion").val();
    ...
    input_data = $("#modalGenPropiedad :input").serializeArray();
    $.each(input_data, function(key, fields) {
        data.append(fields.name, fields.value);
    });
    ...
    $.ajax({
        ...
        success: function(data) {
            var scrollPropiedades;
            var iScroll = 0;
            scrollPropiedades = $("#gvPropiedadSelected .listview");
            iScroll = scrollPropiedades.scrollTop();
            ++indexList;
            progressError = false;
            pbMetro.progressbar("value", 100);
...
function GoToPropiedades(_proyecto) {
    ...
    var nombreproyecto = proyecto.find(".descripcion").text();
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    iditem = data[i].idpropiedad;
                    idtipopropiedad = data[i].idtipopropiedad;
                    if (data[i].numerodoc != null) nrodoc = data[i].numerodoc.trim().length == 0 ? "" : data[i].numerodoc + " ";
                    if (data[i].descrippersona != null) nombrepropietario = data[i].descrippersona.trim().length == 0 ? "(EN BLANCO)" : data[i].descrippersona; else nombrepropietario = "(EN BLANCO)";
...
function GenerarVistaPreviaPropiedad() {
    ...
    var inicio = 0;
    elem = $("#txtNroFinal");
    inicio = parseInt($("#txtNroInicial").val());
        ...
        success: function(data) {
            ...
            if (countdata > 0) {
                while (i < countdata) {
                    ...
                    strhtml += "<td></td>";
                    strhtml += '<td><button class="btn btn-primary center-block">Ver detalle</button></td>';
                    strhtml += "</tr>";
                    ++i;
                }
                ...
                $("#tableProceso .ibody-content").animate({
                    scrollTop: rowpos.top - 50
                }, 400, function() {});
            } else {
...
function ActualizarValorPropiedad() {
    ...
    $("#gvPropiedad .dato").each(function(index, el) {
        ...
        if (tipovaloracion == "02") valor = ratio; else if (tipovaloracion == "03") valor = importefijo;

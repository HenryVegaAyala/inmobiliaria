...
(function (b) {
  b.MetroMessageBox = function (v, a) {
    ...
    if (isIE8orlower() == 0) {
      ...
      i.setAttribute("src", "plugins/metro-alert/sound/messagebox.mp3");
      b.get();
    ...
    for (var p = 0; p <= v.buttons.length - 1; p++) {
      if (v.buttons[p] == "[") {
        ...
        if (v.buttons[p] == "]") {
          r = r + 1;
          q = "<button type='button' id='bot" + r + "-Msg" + MetroMSGboxCount + "' class='botTempo' style='background-color: " + v.NormalButton + ";'> " + q + "</button>";
          u += q
        } else {
          q += v.buttons[p]
        }
...
(function (b) {
  b.bigBox = function (a, h) {
    ...
    if (isIE8orlower() == 0) {
      var i = document.createElement("audio");
      i.setAttribute("src", "plugins/metro-alert/sound/bigbox.mp3");
      b.get();
      i.addEventListener("load", function () {
        i.play()
      }, true);
    ...
    BigBoxes = BigBoxes + 1;
    k = "<div id='bigBox" + BigBoxes + "' class='bigBox animated fadeIn fast'><div id='bigBoxColor" + BigBoxes + "'><img class='botClose' id='botClose" + BigBoxes + "' src='plugins/metro-alert/img/close.png'>";
    k += "<span>" + a.title + "</span>";
    k += "<p>" + a.content + "</p>";
    k += "<div class='bigboxicon'>";
    if (a.img == undefined) {
      a.img = "plugins/metro-alert/img/cloud.png"
    }
    k += "<img src='" + a.img + "'>";
    k += "</div>";
...
(function (b) {
  b.smallBox = function (o, j) {
    ...
    if (isIE8orlower() == 0) {
      ...
      k.setAttribute("src", "plugins/metro-alert/sound/smallbox.mp3");
      b.get();
